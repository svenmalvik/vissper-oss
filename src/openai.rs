...
mod tests {
    ...
    fn test_select_prompt() {
        let basic = PolishConfig::basic_polish();
        assert_eq!(select_prompt(&basic), POLISH_PROMPT);

        let live = PolishConfig::live_meeting();
        assert_eq!(select_prompt(&live), LIVE_MEETING_PROMPT);
    }
}
